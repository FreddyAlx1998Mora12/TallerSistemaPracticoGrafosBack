...
public class GraphDirect extends Graph {
	...
	public GraphDirect(Integer n_vert) {
		...
		for (int i = 1; i <= nro_vertice; i++) {
			listAdyacencia[i] = new MyLinkedList<>(); // Para cada Adyacencia o mejor dicho para cada vertice se crea
	...
	public Boolean is_edge(Integer v1, Integer v2) throws Exception {
		// TODO Auto-generated method stub
		boolean flag = false;
		if (v1.intValue() <= nro_vertice && v2.intValue() <= nro_vertice) { // verifica que los vertices (nodos) pasados
																			// por paramteros no excedan el nro de
																			// vertices (ttoal)
...
//		Float weigFloat = Float.NaN;
	...
	public void add_edge(Integer v1, Integer v2, Float weight) throws Exception {
		...
		if (v1.intValue() <= nro_vertice && v2.intValue() <= nro_vertice) {
			if (!is_edge(v1, v2)) { // Si no son arista, logicamente el elemento v2 no debe constar en v1 para
									// poderse agg
				nro_edg++;
